  ...
  describe "GET #index" do
    ...
    context "with time period filters" do
      ...
      it "handles week filter" do
        ...
        expect(assigns(:time_range).first).to be >= (1.week.ago - 1.second)
      end

      it "handles month filter" do
        get :index, params: { time_period: "month" }
        expect(assigns(:time_range).first).to be >= (1.month.ago - 1.second)
      end

      it "handles quarter filter" do
        get :index, params: { time_period: "quarter" }
        expect(assigns(:time_range).first).to be >= (3.months.ago - 1.second)
      end

      it "handles year filter" do
        get :index, params: { time_period: "year" }
        expect(assigns(:time_range).first).to be >= (1.year.ago - 1.second)
      end
      ...
      it "uses default range for invalid time period" do
        ...
        expect(assigns(:time_range).first).to be >= (30.days.ago - 1.second)
      end
    end
