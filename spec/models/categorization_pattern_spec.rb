...
RSpec.describe CategorizationPattern, type: :model do
  ...
  describe "#matches?" do
    ...
    context "with merchant/keyword/description pattern" do
      ...
      context "when pattern_type is keyword" do
        before do
          pattern.pattern_type = "keyword"
          pattern.pattern_value = "coffee"
        end

        it "matches when keyword is in merchant_name" do
          expense.merchant_name = "Starbucks Coffee"
          expense.description = "Morning beverage"
          expect(pattern.matches?(expense)).to be true
        end

        it "matches when keyword is in description" do
          expense.merchant_name = "Random Store"
          expense.description = "Coffee and pastry"
          expect(pattern.matches?(expense)).to be true
        end

        it "matches when keyword is in both fields" do
          expense.merchant_name = "Coffee Shop"
          expense.description = "Coffee and bagel"
          expect(pattern.matches?(expense)).to be true
        end

        it "does not match when keyword is in neither field" do
          expense.merchant_name = "Random Store"
          expense.description = "Random item"
  ...
  describe "#effective_confidence" do
    ...
    it "reduces confidence for patterns with little data" do
      ...
      # New normalization formula reduces confidence for patterns with little data
      expect(pattern.effective_confidence).to be < 0.6
      expect(pattern.effective_confidence).to be > 0.5
    end
    ...
    it "adjusts confidence based on success rate with enough data" do
      ...
      # New normalization formula with good success rate should be around 0.6
      expect(pattern.effective_confidence).to be > 0.55
      expect(pattern.effective_confidence).to be < 0.65
    end
  end
