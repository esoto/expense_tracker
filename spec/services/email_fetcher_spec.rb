require 'rails_helper'

RSpec.describe EmailFetcher, type: :service do
  let(:email_account) { create(:email_account, :bac) }
  let(:mock_imap_service) { instance_double(ImapConnectionService) }
  let(:mock_email_processor) { instance_double(EmailProcessor) }
  let(:fetcher) { EmailFetcher.new(email_account, imap_service: mock_imap_service, email_processor: mock_email_processor) }

  before do
    allow(mock_imap_service).to receive(:errors).and_return([])
    allow(mock_email_processor).to receive(:errors).and_return([])
  end

  describe '#initialize' do
    it 'sets the email account and initializes empty errors' do
      expect(fetcher.email_account).to eq(email_account)
      expect(fetcher.errors).to be_empty
      expect(fetcher.imap_service).to eq(mock_imap_service)
      expect(fetcher.email_processor).to eq(mock_email_processor)
    end

    it 'creates default services if none provided' do
      default_fetcher = EmailFetcher.new(email_account)
      expect(default_fetcher.imap_service).to be_a(ImapConnectionService)
      expect(default_fetcher.email_processor).to be_a(EmailProcessor)
    end
  end

  describe '#fetch_new_emails' do
    context 'with valid account' do
      before do
        allow(fetcher).to receive(:valid_account?).and_return(true)
      end

      it 'successfully fetches emails' do
        allow(fetcher).to receive(:search_and_process_emails).and_return({
          processed_emails_count: 3,
          total_emails_found: 5
        })
        
        result = fetcher.fetch_new_emails(since: 1.day.ago)
        expect(result).to be_a(EmailFetcherResponse)
        expect(result.success?).to be true
        expect(result.processed_emails_count).to eq(3)
        expect(result.total_emails_found).to eq(5)
      end

      it 'passes the correct since parameter' do
        since_date = 2.days.ago
        expect(fetcher).to receive(:search_and_process_emails).with(since_date).and_return({
          processed_emails_count: 0,
          total_emails_found: 0
        })
        fetcher.fetch_new_emails(since: since_date)
      end

      it 'uses default since of 1 week ago' do
        expect(fetcher).to receive(:search_and_process_emails).with(anything).and_return({
          processed_emails_count: 0,
          total_emails_found: 0
        })
        fetcher.fetch_new_emails
      end
    end

    context 'with invalid account' do
      before do
        allow(fetcher).to receive(:valid_account?).and_return(false)
      end

      it 'returns failure response without attempting connection' do
        expect(fetcher).not_to receive(:search_and_process_emails)
        result = fetcher.fetch_new_emails
        expect(result).to be_a(EmailFetcherResponse)
        expect(result.failure?).to be true
      end
    end

    context 'with IMAP errors' do
      before do
        allow(fetcher).to receive(:valid_account?).and_return(true)
        allow(fetcher).to receive(:search_and_process_emails)
          .and_raise(ImapConnectionService::ConnectionError, 'Connection failed')
      end

      it 'handles IMAP connection errors gracefully' do
        result = fetcher.fetch_new_emails
        expect(result).to be_a(EmailFetcherResponse)
        expect(result.failure?).to be true
        expect(result.errors).to include('IMAP Error: Connection failed')
      end
    end

    context 'with authentication errors' do
      before do
        allow(fetcher).to receive(:valid_account?).and_return(true)
        allow(fetcher).to receive(:search_and_process_emails)
          .and_raise(ImapConnectionService::AuthenticationError, 'Auth failed')
      end

      it 'handles IMAP authentication errors gracefully' do
        result = fetcher.fetch_new_emails
        expect(result).to be_a(EmailFetcherResponse)
        expect(result.failure?).to be true
        expect(result.errors).to include('IMAP Error: Auth failed')
      end
    end

    context 'with unexpected errors' do
      before do
        allow(fetcher).to receive(:valid_account?).and_return(true)
        allow(fetcher).to receive(:search_and_process_emails)
          .and_raise(StandardError, 'Unexpected error')
      end

      it 'handles standard errors gracefully' do
        result = fetcher.fetch_new_emails
        expect(result).to be_a(EmailFetcherResponse)
        expect(result.failure?).to be true
        expect(result.errors).to include('Unexpected error: Unexpected error')
      end
    end
  end

  describe '#test_connection' do
    context 'with successful connection' do
      before do
        allow(mock_imap_service).to receive(:test_connection).and_return(true)
      end

      it 'returns success response for successful connection' do
        result = fetcher.test_connection
        expect(result).to be_a(EmailFetcherResponse)
        expect(result.success?).to be true
      end

      it 'collects errors from imap service' do
        allow(mock_imap_service).to receive(:errors).and_return(['Service error'])
        result = fetcher.test_connection
        expect(result.errors).to include('Service error')
      end
    end

    context 'with connection failure' do
      before do
        allow(mock_imap_service).to receive(:test_connection).and_return(false)
        allow(mock_imap_service).to receive(:errors).and_return(['Connection failed'])
      end

      it 'returns failure response and includes service errors' do
        result = fetcher.test_connection
        expect(result).to be_a(EmailFetcherResponse)
        expect(result.failure?).to be true
        expect(result.errors).to include('Connection failed')
      end
    end
  end

  describe '#search_and_process_emails' do
    let(:since_date) { 1.day.ago }
    let(:message_ids) { [1, 2, 3] }

    before do
      allow(fetcher).to receive(:build_search_criteria).with(since_date).and_return(['SINCE', '01-Jan-2025'])
      allow(mock_imap_service).to receive(:search_emails).and_return(message_ids)
      allow(mock_email_processor).to receive(:process_emails).and_return({
        processed_count: 2,
        total_count: 3
      })
    end

    it 'builds correct search criteria' do
      expect(fetcher).to receive(:build_search_criteria).with(since_date)
      fetcher.send(:search_and_process_emails, since_date)
    end

    it 'delegates email processing to EmailProcessor' do
      expect(mock_email_processor).to receive(:process_emails).with(message_ids, mock_imap_service)
      
      result = fetcher.send(:search_and_process_emails, since_date)
      expect(result[:processed_emails_count]).to eq(2)
      expect(result[:total_emails_found]).to eq(3)
    end

    it 'returns hash when no emails found' do
      allow(mock_imap_service).to receive(:search_emails).and_return([])
      result = fetcher.send(:search_and_process_emails, since_date)
      expect(result[:processed_emails_count]).to eq(0)
      expect(result[:total_emails_found]).to eq(0)
    end

    it 'collects errors from both services' do
      allow(mock_imap_service).to receive(:errors).and_return(['IMAP error'])
      allow(mock_email_processor).to receive(:errors).and_return(['Processing error'])
      
      fetcher.send(:search_and_process_emails, since_date)
      
      expect(fetcher.errors).to include('IMAP error')
      expect(fetcher.errors).to include('Processing error')
    end
  end


  describe '#valid_account?' do
    context 'with active account and password' do
      it 'returns true' do
        expect(fetcher.send(:valid_account?)).to be true
      end
    end

    context 'with inactive account' do
      before { email_account.update(active: false) }

      it 'returns false and adds error' do
        result = fetcher.send(:valid_account?)
        expect(result).to be false
        expect(fetcher.errors).to include('Email account is not active')
      end
    end

    context 'with missing password' do
      before { allow(email_account).to receive(:encrypted_password).and_return(nil) }

      it 'returns false and adds error' do
        result = fetcher.send(:valid_account?)
        expect(result).to be false
        expect(fetcher.errors).to include('Email account missing password')
      end
    end
  end

  describe '#build_search_criteria' do
    it 'creates SINCE criteria with formatted date' do
      since_date = Date.new(2025, 1, 15)
      criteria = fetcher.send(:build_search_criteria, since_date)
      expect(criteria).to eq(['SINCE', '15-Jan-2025'])
    end
  end

end