...
RSpec.describe BulkCategorizationActionsController, type: :controller, unit: true do
  ...
  let(:bulk_categorization_service) { instance_double(Categorization::BulkCategorizationService) }
  ...
  describe "error handling", unit: true do
    context "when service raises a standard error" do
      before do
        ...
        error_service = instance_double(Categorization::BulkCategorizationService)
        allow(Categorization::BulkCategorizationService).to receive(:new).and_return(error_service)
