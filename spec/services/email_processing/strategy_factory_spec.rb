...
RSpec.describe Services::EmailProcessing::StrategyFactory, integration: true do
  ...
  describe '.create_strategy', integration: true do
    it 'creates default strategy for unknown banks' do
      ...
      expect(strategy).to be_a(Services::EmailProcessing::Strategies::Regex)
    ...
    context 'with email content validation' do
      it 'logs warning when strategy cannot parse content' do
        ...
        expect(Rails.logger).to receive(:warn).with(
          "[Services::EmailProcessing::StrategyFactory] Strategy #{Services::EmailProcessing::Strategies::Regex} cannot parse content for #{parsing_rule.bank_name}"
        )
    ...
    context 'with bank-specific strategies' do
      before do
        ...
        stub_const('ParsingStrategyFactory::STRATEGY_MAPPINGS', {
          'banco_nacional' => Services::EmailProcessing::Strategies::Regex,
          'custom_bank' => Services::EmailProcessing::Strategies::Regex,
  ...
  describe '.strategy_for_bank', integration: true do
    it 'returns default strategy for unknown banks' do
      ...
      expect(strategy_class).to eq(Services::EmailProcessing::Strategies::Regex)
    end
    ...
    context 'with custom bank mappings' do
      before do
        stub_const('ParsingStrategyFactory::STRATEGY_MAPPINGS', {
          'special_bank' => Services::EmailProcessing::Strategies::Regex,
          default: Services::EmailProcessing::Strategies::Regex
        })
      end
  ...
  describe 'constants', integration: true do
    it 'defines strategy mappings as frozen hash' do
      ...
      expect(described_class::STRATEGY_MAPPINGS[:default]).to eq(Services::EmailProcessing::Strategies::Regex)
    end
  end
end
